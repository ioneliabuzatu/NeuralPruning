...
from utils_new import prune_utils
from test import evaluate


class SENSITIVITY:
  def __init__(self, model, batch_norm_idx, conv_idx, to_prune_idx, testset_path, class_names, pruning_percentile, prune_single_layer):
    self.model = model
    self.all_masks = []
    self.num_filters = []
  ...
  def build_pruned_model(self):
    ...
    self.pruned_tmp = prune_utils.prune_model_keep_size(self.model, self.to_prune_idx, self.batch_norm_idx, self.modules2masks)
    ...
    prune_utils.init_weights_from_loose_model(self.pruned_tmp, self.pruned_final, self.batch_norm_idx, self.conv_idx,
                                              self.modules2masks)

  def evaluate_model_(self, model):
    eval = evaluate(model, path = self.testset_path, iou_thres=0.5, conf_thres=0.001, nms_thres=0.5, img_size=416, batch_size=1)
    return eval
  ...
  def get_params_by_layer(self):
    for layer_id, layer in enumerate(self.pruned_final.module_list):
      if isinstance(layer[0], torch.nn.Conv2d):
        ...
        # if tot_original_params == tot_pruned_params:
        print(
            f"Layer num {layer_id} | Original params: {tot_original_params} | Pruned {tot_pruned_params}")
