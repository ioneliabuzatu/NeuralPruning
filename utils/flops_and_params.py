...
def get_model_complexity_info(model, input_res, print_per_layer_stat=True, as_strings=True,
                              input_constructor=None, output_flops_no_model=True):
    ...
    if input_constructor:
        ...
        batch = torch.FloatTensor(1, *input_res) # torch.cuda.FloatTensor
        _ = flops_model(batch)
...
def print_model_with_flops(model, units='GMac', precision=3):
    ...
    def flops_repr(self):
        ...
        print(', '.join([flops_to_string(accumulated_flops_cost, units=units, precision=precision),
    ...
    def add_extra_repr(m):
        ...
        flops_per_layer = flops_to_string(accumulate_flops(m), 'GMac',3)
        macs = '{:.3%}'.format(accumulate_flops(m)/total_flops)
        # print(type(m), flops_per_layer, macs)
        if isinstance(m, torch.nn.modules.container.Sequential):
            layers_name_inside_module = [i for i in m._modules.keys()]
            type_first = type(m._modules[layers_name_inside_module[0]])
            name_layer = [i for i in m._modules.keys()][0]
            if 'shortcut' not in name_layer and 'yolo' not in name_layer and 'upsample' not in name_layer and 'route' not in name_layer and len(layers_name_inside_module) == 3:
                all_flops.append(1)
                # print(len(all_flops), name_layer, flops_per_layer, macs)
                # print(name_layer, "|", flops_per_layer.split()[0], "|", macs)
                output.append(f"{name_layer} | {flops_per_layer.split()[0]} | {macs}")

        round(accumulate_flops(m) / 10. ** 9, 3)
...
def compute_average_flops_cost(self):
    ...
    for module in self.modules():
        if is_supported_instance(module):
            ...
            # print(module.__flops__/batches_count)
